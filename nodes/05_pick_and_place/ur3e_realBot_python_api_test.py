...
input(" Go to Red Lego Position => Enter")
joint_goal = group.get_current_joint_values()
joint_goal[0] = np.deg2rad(137)       # shoulder pan
joint_goal[1] = np.deg2rad(-69)      # shoulder lift
joint_goal[2] = np.deg2rad(94)      # elbow
joint_goal[3] = np.deg2rad(-114)     # wrist1
joint_goal[4] = np.deg2rad(-85)     # wrist2
joint_goal[5] = np.deg2rad(89)      # wrist3
...
input(" Go to red Box Position (UR3e) => Enter")
# Position for the UR3e - Table
joint_goal = group.get_current_joint_values()
joint_goal[0] = np.deg2rad(85)       # shoulder pan / Basis
joint_goal[1] = np.deg2rad(-34)       # shoulder lift / Schulter
joint_goal[2] = np.deg2rad(2)      # elbow
joint_goal[3] = np.deg2rad(-55)     # wrist1
joint_goal[4] = np.deg2rad(-86)     # wrist2
joint_goal[5] = np.deg2rad(39)      # wrist3
group.go(joint_goal, wait=True)
